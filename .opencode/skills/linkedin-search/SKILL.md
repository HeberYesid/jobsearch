...
```bash
bun run .opencode/skills/linkedin-search/cli/src/cli.ts search --location "<place>" [flags]
```

Key flags:
- `--location <text>` / `-l <text>` — **required.** A LinkedIn place string, e.g. `"Bogotá, Colombia"`, `"Berlin, Germany"`, `"London, United Kingdom"`, or `"Remote"`.
...
```bash
bun run .opencode/skills/linkedin-search/cli/src/cli.ts detail <id|url> [--format json|plain]
```
...
```bash
# Software engineer roles in Bogotá, last 30 days
bun run .opencode/skills/linkedin-search/cli/src/cli.ts search -q "software engineer" -l "Bogotá, Colombia" --jobage 30 --format table

# Product manager roles in Berlin, remote
bun run .opencode/skills/linkedin-search/cli/src/cli.ts search -q "product manager" -l "Berlin, Germany" --remote remote --format table

# Any role, fully remote
bun run .opencode/skills/linkedin-search/cli/src/cli.ts search -q "data analyst" -l "Remote" --format table

# Full details for a specific job
bun run .opencode/skills/linkedin-search/cli/src/cli.ts detail 4426311357 --format plain
```
