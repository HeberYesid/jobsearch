...
description: >
  ...
  (LinkedIn, freehire, and any skills added with /add-portal). Deduplicates
  across runs. Triggers on: job scrape, find jobs, search jobs, new jobs, job
...
Searches job portals using the **installed portal-search CLIs** in
`.opencode/skills/` (plus WebSearch as a fallback), using queries from your
profile. It deduplicates against previously seen jobs and the application
tracker, and presents new matches with a quick fit assessment.
...
Optional arguments:
- A focus area, e.g. "/scrape data science" or "/scrape backend"
...
2. Read `job_search_tracker.csv` if it exists, to extract already-applied companies+roles.
3. Read `profile/search-queries.md` for the search strategy. If this file does not exist, prompt the user to run `/setup` first and stop.
...
Discover all installed portal CLI skills by reading every `SKILL.md` found under `.opencode/skills/*/SKILL.md`. Each file documents that portal's exact CLI flags and usage examples. **Use each portal's own documented interface — do not guess flags.** This approach automatically includes any new portals added via `/add-portal` without requiring changes to this file.

For each installed portal skill:

1. Read its `SKILL.md` to find the correct `bun run …` invocation and supported flags.
2. Translate the query terms from `search-queries.md` into that portal's flag format (e.g. `--query`, `--location`, `--region`, `--remote` — whatever the portal's SKILL.md specifies).
3. Scope to the last 14 days using the portal's supported recency flag (`--jobage`, etc. — as documented per portal).
4. Cap results to ~20 per call using the portal's limit flag.
5. Use `--format json` for machine-readable output.

Run all portal CLI calls in parallel where possible. Collect all `results` arrays into a single pool for Step 2, keeping each result tagged with its source portal skill (for Step 2 `detail` lookups).
...
Use `WebSearch` for:
- Portals listed in `search-queries.md` that do **not** have a corresponding directory under `.opencode/skills/`
...
Future commands (`/rank`, `/outcome`) extend this schema additively: ranked entries also carry `rank_score` (0–100), `rank_verdict` (fit band), and `rank_date`. Do not drop these fields when re-writing entries.
...
If no new jobs are found, state "No new positions found."
...
3. **Focus on configured geographic area.** Skip jobs that require relocation or are clearly outside commute range (unless the user's `search-queries.md` explicitly includes remote/international).
