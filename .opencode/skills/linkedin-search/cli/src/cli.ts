...
SEARCH FLAGS
  --location, -l <text>   Location to search. REQUIRED. e.g. "Bogotá, Colombia",
...
EXAMPLES
  bun run src/cli.ts search -q "data engineer" -l "Bogotá, Colombia" --jobage 30 --format table
  bun run src/cli.ts search -q "product manager" -l "Berlin, Germany" --remote remote --format table
  bun run src/cli.ts search -q "software engineer" -l "Remote" --format table
  ...
  if (cmd === "search") {
    ...
    if (!location) {
      process.stderr.write(
        JSON.stringify({
          error: 'the --location/-l flag is required (e.g. -l "Bogotá, Colombia", -l "Berlin, Germany", or -l "Remote")',
          code: "NO_LOCATION",
        }) + "\n",
      )
      return 1
