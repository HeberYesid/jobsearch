...
## Hosted-service dependency (best-effort, no SLA)
...
```bash
FREEHIRE_API_URL=http://localhost:8080 bun run .opencode/skills/freehire-search/cli/src/cli.ts search -q "go"
```
...
```bash
bun run .opencode/skills/freehire-search/cli/src/cli.ts search [-q "<keywords>"] [facet flags]
```
...
- `--region <codes>` — macro-region, e.g. `global`, `eu`, `us`, `apac`, `latam`, `cis`. `--region eu,us`. Use `none` to match jobs whose region could **not** be resolved.
...
```bash
bun run .opencode/skills/freehire-search/cli/src/cli.ts detail <slug|url> [--format json|plain]
```
...
bun run .opencode/skills/freehire-search/cli/src/cli.ts search -q "backend engineer" --seniority senior --limit 10 --format table

# Remote React roles in LatAm
bun run .opencode/skills/freehire-search/cli/src/cli.ts search -q "react" --remote remote --region latam --format table

# DevOps roles in Colombia posted in the last 14 days
bun run .opencode/skills/freehire-search/cli/src/cli.ts search --category devops --country CO --jobage 14 --format table

# ML/AI roles anywhere, fully remote
bun run .opencode/skills/freehire-search/cli/src/cli.ts search -q "machine learning" --category ml_ai --remote remote --format table

# Full details for a specific job
bun run .opencode/skills/freehire-search/cli/src/cli.ts detail golang-zensar-2bxu6dxm --format plain
```
